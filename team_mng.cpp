...
{
	int selectedRow = ui.tableWidget_2->currentRow();  // 获取选中行号
	if (selectedRow < 0) return;
	...
	//安全删除用户的创建团队文件内的对应数据行
	QFile file_usr_create(users[USR_ID_NOW - USER_ID_FORE].usr_team_create_filename);
	if (file_usr_create.open(QIODevice::ReadWrite | QIODevice::Text)) {
		//输出打印teams[team_id - TEAM_ID_FORE].team_members中的内容
		qDebug() << "teams[team_id - TEAM_ID_FORE].team_members中的内容：\n";
		for (int i = 0; i < teams[team_id - TEAM_ID_FORE].team_members.size(); i++) {
			qDebug() << teams[team_id - TEAM_ID_FORE].team_members[i] << "\n";
		...
		//安全删除用户的创建团队文件内的对应数据行
		QTextStream stream(&file_usr_create);
		/*
		3
		2001,SCUT,1001,2,1001,1002,
		2002,SE,1001,2,1001,1002,
		...
		*/
		...
		// 清空文件内容
		file_usr_create.resize(0);
		// 重新写入更新后的数据
		stream << lines.join("\n");
		file_usr_create.close();
	}
	else {
		QMessageBox::critical(this, "error", "usr_team_create_filename file open fail");
		return;
	}

	//删除团队成员的usr_team_belong文件中的对应行
	/*
		3
		2001,SCUT,1001,2,1001,1002,
		2002,SE,1001,2,1001,1002,
		2003,QBB,1002,1,1002,
	*/
	//打开当前删除的团队的所有成员的usr_team_belong文件
	for (int i = 0; i < teams[team_id - TEAM_ID_FORE].team_members.size(); i++) {
		QFile file_usr_belong(users[teams[team_id - TEAM_ID_FORE].team_members[i] - USER_ID_FORE].usr_team_belong_filename);
		if (file_usr_belong.open(QIODevice::ReadWrite | QIODevice::Text)) {
			QTextStream stream(&file_usr_belong);
			QStringList lines; // 保存文件所有数据
			while (!stream.atEnd()) {
				...
				QString line;
				line = stream.readLine().trimmed();
				//找到了数据行的第一个字段和team_id相等，就跳过
				if (line.size() >= 2 && line.split(",").at(0).toInt() == team_id) continue;
				lines.append(line);
			}
			// 清空文件内容
			file_usr_belong.resize(0);
			...
			QMessageBox::critical(this, "error", "usr_team_belong_filename file open fail");
			return;
		}
	}

	//删除"TEAMS.txt"文件中的对应行
	QFile file_teams("TEAMS.txt");
	/*
	2001,学习小组,111,1001,1,0,2001_team members.txt,2001_team events.txt
	2002,学习小组,111,1001,1,0,2001_team members.txt,2001_team events.txt
	*/
	if (file_teams.open(QIODevice::ReadWrite | QIODevice::Text)) {
		QTextStream stream(&file_teams);
		QStringList lines; // 保存文件所有数据
		int currentrow = -1;//抵消掉第一行的团队数量
		while (!stream.atEnd()) {
			QStringList lines;
		...
		QMessageBox::critical(this, "error", "TEAMS.txt file open fail");
		return;
	}
	//删除该团队的附带文件
	//删除"2001_team members.txt"文件
	QString team_members_filename = QString::number(teams[team_id - TEAM_ID_FORE].team_id) + "_team members.txt";
	qDebug() << "team_members_filename" << team_members_filename;
	QFile file_team_members(team_members_filename);
	if (file_team_members.open(QIODevice::ReadWrite | QIODevice::Text)) {
		file_team_members.resize(0);
		file_team_members.close();
	}
	else {
		//QMessageBox::critical(this, "error", "team_members_filename.txt file open fail");
		return;
	}
	//删除"2001_team events.txt"文件
	QString team_events_filename = QString::number(teams[team_id - TEAM_ID_FORE].team_id) + "_team events.txt";
	qDebug() << "team_events_filename" << team_events_filename;
	QFile file_team_events(team_events_filename);
	if (file_team_events.open(QIODevice::ReadWrite | QIODevice::Text)) {
		file_team_events.resize(0);
		file_team_events.close();
	}
	else {
		QMessageBox::critical(this, "error", "team_events_filename file open fail");
		return;
	}
	//删除"2001_AD.txt"
	QString AD_filename = QString::number(teams[team_id - TEAM_ID_FORE].team_id) + "_AD.txt";
	QFile file_AD(AD_filename);
	if (file_AD.open(QIODevice::ReadWrite | QIODevice::Text)) {
		file_AD.resize(0);
		file_AD.close();
	}
	else {
		QMessageBox::critical(this, "error", "AD.txt file open fail");
		return;
	}
	...
	ui.tableWidget->removeRow(selectedRow);
	//提示用户退出成功
	QMessageBox::information(this, "tips", "delete team success");
}
team_mng::~team_mng()
{
}
...
void team_mng::confirm_join(const int& team_id, const QString& team_password)
{
	...
	else
	{
		...
		qDebug() << "teams[team_id - TEAM_ID_FORE].team_members_nums:" << teams[team_id - TEAM_ID_FORE].team_members_nums << "\n";
		//1
		...
		for (int i = 0; i < teams[team_id - TEAM_ID_FORE].team_members_nums; i++)
		{
			qDebug() << "第" << i << "位成员所属的团队数量：" << users[teams[team_id - TEAM_ID_FORE].team_members[i] - USER_ID_FORE].usr_team_belong_num << "现在用户：" << USR_ID_NOW << "\n";
			if (users[teams[team_id - TEAM_ID_FORE].team_members[i] - USER_ID_FORE].usr_id == USR_ID_NOW)
			{
	...
	*/
	qDebug() << "下面进行【加入团队】的【TEAMS.txt】操作！";
	QFile teams_file("TEAMS.txt");
	if (!teams_file.open(QIODevice::ReadWrite | QIODevice::Text | QIODevice::Append))
	{
		qDebug() << "打开TEAMS.txt失败！";
		return;
	}
	qDebug() << "打开【TEAMS.txt】成功！";
	QTextStream teams_file_stream(&teams_file);
	//如果团队当前ID是2001，则读取文件第0行，以此类推
	teams_file_stream.seek(0);
	int lineindex = 0;
	while (!teams_file_stream.atEnd())
	{
		qDebug() << "开始定位需要修改的团队数据行";
		if (lineindex == (team_id - TEAM_ID_FORE))
		{
			//找到了对应的团队
			//更新成员数量
			teams_file_stream.seek(0);
			QString line;
			line = teams_file_stream.readLine();
			QStringList list = line.split(",");
			qDebug() << "找到了，teams[team_id - TEAM_ID_FORE].team_members_nums:" << teams[team_id - TEAM_ID_FORE].team_members_nums;
	...
	qDebug() << "下面进行【加入团队】的【USERS.txt】操作";
	//更新该团队中所有成员在从属团队文件中存储的该团队的人员数量
	...
	int lineindex_USES = 0;
	while (!users_file_stream.atEnd())
	{
		if (lineindex_USES == USR_ID_NOW - USER_ID_FORE - 1)
		{
			//读取当前用户的信息
			QRegularExpression re(",$");
			...
			// 将更新后的数据重新写入文件
			users_file_stream.seek(lineindex_USES * (line.size() + 1)); // 定位到当前行的开头
	...
	QString file_name_usr_belong = users[USR_ID_NOW - USER_ID_FORE].usr_team_belong_filename;
	QFile now_usr_belong(file_name_usr_belong);
	QTextStream out_usr_now_belong(&now_usr_belong);
	...
	out_usr_now_belong << users[USR_ID_NOW - USER_ID_FORE].usr_team_belong_num << ",";
...
void team_mng::updateTable_team()
{
	...
	if (file_belong.open(QIODevice::ReadOnly | QIODevice::Text)) {
		...
		ui.tableWidget->setHorizontalHeaderLabels(QStringList() << "team id" << "team name" << "founder" << "member num");
		int row = 0; // 行计数器
		while (!in.atEnd()) {
			QString line = in.readLine().trimmed();
			QStringList fields = line.split(",");
	...
	if (ui.tableWidget->rowCount() == 0) {
		...
		connect(ui.tableWidget, SIGNAL(doubleClicked(QModelIndex)), this, SLOT(on_doubleClicked_belong()));
	}
	//如果当前表格为空，表格中添加一行消息：当前您没有该类型的团队！
	if (ui.tableWidget_2->rowCount() == 0) {
		...
		//如果被双击，提示请创建团队
		connect(ui.tableWidget_2, SIGNAL(doubleClicked(QModelIndex)), this, SLOT(on_doubleClicked_create()));
	}
	qDebug() << "updateTable_team();结束！";
}
...
{
	//如果此时表头为一列，提示请先创建团队，然后返回
	if (ui.tableWidget->columnCount() == 1) {
		QMessageBox::information(this, "提示", "请先加入团队！");
		return;
	}
	//双击QTableWidget行，进入该行对应的team_id，并且传入打开的团队日历窗口
...
void team_mng::on_doubleClicked_create()
{
	...
	if (ui.tableWidget_2->columnCount() == 1) {
		QMessageBox::information(this, "提示", "请先创建团队！");
		return;
	}
	//双击QTableWidget行，进入该行对应的team_id，并且传入打开的团队日历窗口
